...
/*
 *  Constructs a rptWoScheduleByParameterList as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 *  The dialog will by default be modeless, unless you set 'modal' to
 *  true to construct a modal dialog.
 */
rptWoScheduleByParameterList::rptWoScheduleByParameterList(QWidget* parent, const char* name, bool modal, Qt::WFlags fl)
    : QDialog(parent, name, modal, fl)
{
    setupUi(this);

    QButtonGroup* _sortByGroupInt = new QButtonGroup(this);
    _sortByGroupInt->addButton(_sortByStartDate);
    _sortByGroupInt->addButton(_sortByDueDate);
    _sortByGroupInt->addButton(_sortByItemNumber);
 ...
 */
rptWoScheduleByParameterList::~rptWoScheduleByParameterList()
{
 ...
 *  language.
 */
void rptWoScheduleByParameterList::languageChange()
{
    retranslateUi(this);
}


void rptWoScheduleByParameterList::init()
{
  _captive = FALSE;
...
enum SetResponse rptWoScheduleByParameterList::set(ParameterList &pParams)
{
  _captive = TRUE;
  ...
  param = pParams.value("wrkcnt_pattern", &valid);
  if (valid)
  {
    _parameter->setType(WorkCenter);
    _parameter->setPattern(param.toString());
  ...
  param = pParams.value("showOnlyRI", &valid);
  _showOnlyRI->setChecked(valid);
  ...
  {
    case ClassCode:
      setCaption(tr("Report Parameters - W/O Schedule by Class Code"));
      break;

    case PlannerCode:
      setCaption(tr("Report Parameters - W/O Schedule by Planner Code"));
      break;

    case ItemGroup:
      setCaption(tr("Report Parameters - W/O Schedule by Item Group"));
      break;

    case WorkCenter:
      setCaption(tr("Report Parameters - W/O Schedule by Work Center"));
      break;
...
void rptWoScheduleByParameterList::sPrint()
{
